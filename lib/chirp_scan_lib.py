...
def sweep_back_forward(prism_x, axis_z, ps5000a, zs0, n_sweeps, wd, last_run_dict, fcs):
    t0 = time.time()
    dict_to_save = {}
    for i in range(n_sweeps):
        minz = last_run_dict['fit']['z0']
        ...
        for z in zs:
            ...
            ps5000a.configurePSD(1, int(1953125/5))
            ps5000a.ChangeRangeTo('A', 1000)
            balancer_prism(ps5000a, prism_x, 'A', -1, 0.005, offset = 0, debug = False)
            ps5000a.ChangeRangeTo('A', 2000)

            if np.absolute(z-z_last) > 0.32:
            ...
            print(zs_measured[-1])
            z_last = axis_z.Position()
        dict_to_save = process_signal_fft(np.array(zs_measured), signal1s, signal0s, idxs)
        j = len(os.listdir(wd))
        dict_to_save['maxfs'] = maxfs
        dict_to_save['fordata'] = fordata
        dict_to_json(dict_to_save, wd, 'result_{:d}.json'.format(j))
        print(time.time() - t0)
...
def PID(setPoint, kp, ki, kd, fc, ps5000a, monitorTime = None):
    ...
    while InRange < 10:
        ...
        if i == 0:
            ...
            sum = sum + et[-1]
            ps = ps + [np.mean(amps)]
        DC = 0.5 - kp*et[-1] - ki*Ti*sum - kd*(et[-1] - et[-2])/(t[-1] - t[-2])
        if DC > 0 and DC < 1:
            ps5000a.SigGenDC(DC)
            DCs = DCs + [DC]
            InRange = 0
...
def fft_process(TS, idx, fs):
    fftch = ms.Single_sided_fft(TS)/np.sqrt(len(TS)*fs)
    ffts = []
